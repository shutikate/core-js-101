...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const y4 = rect2.top;
  return (!(x3 > x2 || y3 < y2 || x1 > x4 || y4 > y1));
}
...
function isCreditCardNumber(ccn) {
  const ccnReverse = String(ccn).split('').reverse().join('');
  let sum = 0;
  for (let i = 0; i < ccnReverse.length; i += 1) {
    if (i % 2 !== 0) {
      sum += (Number(ccnReverse[i]) * 2 > 9)
        ? (Number(ccnReverse[i]) * 2 - 9)
        : Number(ccnReverse[i]) * 2;
    } else {
      sum += Number(ccnReverse[i]);
    }
  }
  return sum % 10 === 0;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (arr.length !== 0 && str[i] === brackets[arr[arr.length - 1]]) {
      ...
      arr.push(bracketIndex + 1);
    } else { arr.push(bracketIndex); }
  }
  return arr.length === 0;
}
